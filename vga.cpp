...
void vga_init() {
    word length = vga_width * vga_height;
    vga_empty_buffer = (byte far*)farmalloc(length);
    if (vga_empty_buffer == 0) {
        cout << "VGA buffer couldn't initialize";
        getch();
    }

    memset(vga_empty_buffer, 9, length);

    blit2page(vga_empty_buffer, vga_page[1], 0, 0, vga_width, vga_height);
    blit2page(vga_empty_buffer, vga_page[2], 0, 0, vga_width, vga_height);
...
void blit2page( byte far *s, word page, int x, int y, int w, int h )
{
    ...
    for( p = 0; p < 4; p++ ) {
        ...
        for(j=0; j<h; j++)
        {
            memcpy(VGA + page + screen_offset, s + bitmap_offset, w >> 2);
            bitmap_offset += w >> 2;
...
void swap_vga() {
    blit2page(vga_empty_buffer, vga_page[vga_current_page + 1], 0, 0, vga_width, vga_height);
    vga_current_page = !vga_current_page;
    page_flip(&vga_page[vga_current_page], &vga_page[vga_current_page + 1]);
}